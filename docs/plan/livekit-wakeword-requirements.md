...
dispatch後も`ON`を維持する場合は、dispatch完了時にセッション無活動タイマーを
リセットする。同じ無音区間に対してターン確定と`OFF`遷移を連続実行してはならない。
...
- dispatchした同じ無音判定内で直ちに`OFF`へ遷移しない。
