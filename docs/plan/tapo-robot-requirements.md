...
- wake判定は専用ONNXモデルを既定とし、STT文字列判定を互換方式として残すこと。
- stop判定はON状態の文字起こし結果で行うこと。
- 詳細は`docs/plan/livekit-wakeword-requirements.md`に従うこと。

### FR-05 ウェイク/スリープ状態管理

- 既定の「ねぇ、ヤタガラス」を検出すると、即時フィードバックを経て
  `OFF -> WAKING -> ON`と遷移すること。
- Sleepワードで `ON -> OFF` 遷移すること。
- `ON` 状態で命令を待ったまま無音が一定時間続いた場合は
  `OFF` に戻ること（`LISTEND_SILENCE_TIMEOUT_SEC`、既定3秒）。
- `ON` 状態で無音が一定時間続いたら、1ターン確定して dispatch できること（設定値、既定3秒）。
- 1ターン確定には`LISTEND_SESSION_END_SILENCE_SEC`を使用し、
  命令未入力のキャンセルとは責務を分離すること。
...
- 初期状態は `OFF`。
- `OFF` でwakeワード検知時、`WAKING`へ遷移して即時フィードバックを再生する。
- `WAKING`で回り込み抑制後、`ON`へ遷移して命令待機を開始する。
- `ON` で sleepワード検知時、`OFF` に遷移しセッション終了。
- `ON` で無音タイムアウト（既定3秒）時、`OFF` に遷移。
