...
- RTSPから音声を取り込み、`faster-whisper`（初期値 `base`）で文字起こしできること。
...
- `ON` 状態で無音がデフォルト30秒続いた場合は `OFF` に戻ること。
...
- `~/.config/yatagarasu/.env` と `~/.config/yatagarasu/.config.yml` に集約すること。
